...
def plot_double_confusion_matrices(
    datasens, datanosens, label, classes, dataset, debiaser, sensitivefeature=None
):
    ...
    if sensitivefeature:
        ...
        plt.savefig(
            "confusionmatrices/cm_" + str(debiaser) + "_" + str(dataset) + ".pdf"
        )
...
def generatecm(dataset, debiaser=None, normalize=False, sensitivefeature=None):
    (
        ...
        unpriv_group,
        k,
    ) = getdataset.getdataset(dataset, 2, sensitivefeature=sensitivefeature)
    newdata = getprediction(
        data,
        label,
        classifier,
        unpriv_group,
    ...
    query = "&".join([str(k) + "==" + str(v) for k, v in unpriv_group.items()])
    datasens = newdata.query(query)
    datanosens = newdata.query("~(" + query + ")")
    plot_double_confusion_matrices(
        datasens,
        datanosens,
        label,
        classes=classes,
